...
function Reports() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:4000/report/totalRevenue").then((response) => {
    ...
    Axios.get("http://localhost:4000/flight/routes").then((response) => {
  ...
  const handlePassengerCount = (event) => {
    ...
    Axios.post("http://localhost:4000/report/passengerCount", {
  ...
  const handleBookingCount = (event) => {
    event.preventDefault();
    Axios.post("http://localhost:4000/report/bookingCount", {
      destination: newDestination.slice(0, 3),
      startDate: departDate,
      endDate: returnDate,
    }).then((response) => {
  ...
  const handlePastFlights = (event) => {
    event.preventDefault();
    Axios.post("http://localhost:4000/report/pastFlights", {
  ...
  const handlePassengerbyAge = (event) => {
    event.preventDefault();
    Axios.post("http://localhost:4000/report/passengerByAge", {
      flight_no: flightNo,
    }).then((response) => {
