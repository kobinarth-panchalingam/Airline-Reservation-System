...
export const airportController = {
    ...
    updateAirportByCode: async (req, res) => {
        ...
        try {
            ...
            const update_data = {}
            const airport_result = await AirportModel.getByCode(
                client,
                airport_code
            )
            let city_id = airport_result[0].location_id
            let city_result = await LocationModel.getById(client, city_id)
            let state_result = await LocationModel.getById(
                client,
                city_result.parent_id
            )
            let country_result = await LocationModel.getById(
                client,
                state_result.parent_id
            )
            ...
            if (!is_country_same) {
                const updated_country = await LocationModel.getByNameAndLevel(
                    client,
                    country,
                    'country'
                )
                ...
                state_result = await LocationModel.update(
                    client,
                    state,
                    state_parent_id,
                    'state',
            ...
            if (!is_state_same) {
                const updated_state = await LocationModel.getByNameAndLevel(
                    client,
                    state,
                    'state'
                )
                ...
                city_result = await LocationModel.update(
                    client,
                    city,
                    city_parent_id,
                    'city',
            ...
            if (!is_city_same) {
                const updated_city = await LocationModel.getByNameAndLevel(
                    client,
                    city,
                    'city'
                )
