...
router.post("/book", (req, res) => {
  ...
  const sqlInsert =
    "insert into booking(booking_id, amount, user_id, flight_id, booked_date, status, count, seat_class) values (?,?,?,?,now(),?,?,?)";
  db.query(sqlInsert, [uuid, amount, user_id, id, "unpaid", ticketInfo.noOfPassengers, ticketInfo.class], (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(uuid);
...
router.get("/bookingDetails/:id", (req, res) => {
  const { id } = req.params;
  const getBooking =
    "select b.booking_id, b.flight_id, b.seat_class, b.amount, DATE_FORMAT(b.booked_date , '%Y-%m-%d | %h:%i:%p') as booked_date, t.ticket_id, t.seat_no, p.passenger_name, p.passport_number  from ticket t left join booking b on t.booking_id = b.booking_id left join passenger p on t.passport_number = p.passport_number  where b.booking_id=?";
  console.log(id);
