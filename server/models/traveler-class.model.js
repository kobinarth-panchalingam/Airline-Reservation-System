...
export const TravelerClass = {
    getAll: async () => {
        try {
            const result = await db.query('SELECT * FROM traveler_class')
            return result.rows
        } catch (error) {
    ...
    getById: async id => {
        try {
            const result = await db.query(
                'SELECT * FROM traveler_class WHERE id = $1',
                [id]
            )
    ...
    create: async data => {
        try {
            const result = await db.query(
                'INSERT INTO traveler_class (class_name) VALUES ($1) RETURNING *',
                [data.class_name]
            )
            return result.rows[0]
        } catch (error) {
    ...
    deleteById: async id => {
        try {
            const result = await db.query(
                'DELETE FROM traveler_class WHERE id = $1 RETURNING *',
                [id]
            )
            return result.rows[0]
        } catch (error) {
